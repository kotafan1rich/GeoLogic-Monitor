...
// HTTPOptions описывает один HTTP-клиент. Клиенты различаются пулом соединений,
// таймаутом, авторизацией и собственным rate limiter'ом, поэтому под каждого
// потребителя (геокодинг, парсинг, запись) создаётся отдельный экземпляр.
type HTTPOptions struct {
	// Name используется только в сообщениях об ошибках.
	Name                string
	AuthToken           string
	RequireAuth         bool
	MaxIdleConns        int

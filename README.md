...
Пользовательские и внутренние HTTP-методы API, авторизация Mini App по MAX initData, расчёт при создании точки, история рейтинга и плановый пересчёт реализованы. Frontend Mini App остаётся целевой возможностью.
...
В `api` реализованы миграции постоянных данных, методы OpenAPI, сервисная и Mini App-авторизация, геокодинг, расчёт рейтинга через OSRM, его история и плановый пересчёт. Полный пользовательский сценарий также зависит от ещё не завершённых `bot`, `ingestion` и frontend Mini App.

...
MVP позволяет добавить несколько точек, посмотреть список и удалить точку. После выбора адреса API рассчитывает рейтинг окружения по пешеходным расстояниям OSRM, возвращает его как дополнительную информацию и включает мониторинг.

## Архитектура и стек

Три независимых Go-модуля: `api` хранит постоянные данные и рассчитывает рейтинг, `ingestion` со своей БД загружает окружение и публикует сигналы в Kafka, `bot` обслуживает MAX Long Polling и отправляет сообщения из Kafka.
